"""In-browser stub for the spaCy model package ``da_core_news_md``.

The real spaCy model packages cannot install in the Pyodide (in-browser)
runtime. Notebooks load a model two ways — ``spacy.load("da_core_news_md")`` and
``import da_core_news_md; nlp = da_core_news_md.load()``. The ``spacy`` shim handles the first; this
...
def load(**kwargs):
    return spacy.load("da_core_news_md")
