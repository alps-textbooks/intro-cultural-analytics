"""In-browser stub for the spaCy model package ``pt_core_news_md``.

The real spaCy model packages cannot install in the Pyodide (in-browser)
runtime. Notebooks load a model two ways — ``spacy.load("pt_core_news_md")`` and
``import pt_core_news_md; nlp = pt_core_news_md.load()``. The ``spacy`` shim handles the first; this
